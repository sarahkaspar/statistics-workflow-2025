...
chow <- mice %>% filter(Diet =="chow")
hf <- mice %>% filter(Diet == "hf")
mytes <- t.test(chow$Bodyweight, hf$Bodyweight, var.equal = TRUE)
mytes
mytes$stderr
```


```{r compare-to-t-distribution, include=FALSE, echo=FALSE}
ts <- seq(-3,3,by=0.05)
dts <- dt(ts, df=22)

data.frame(t = ts, dt=dts) %>% 
  ggplot(aes(t,dt))+
  geom_line()+
  theme_bw()+
  geom_vline(xintercept=mytes$statistic, color="blue")
```
...
```{r echo=FALSE}
before <- paired_mice %>% filter(Time=="before") %>% pull(Bodyweight)
after <- paired_mice %>% filter(Time=="after") %>% pull(Bodyweight)
```
